...
//   - "Social Media (Mastodon)" — mastodonIngest.js, removed from the
//     project directly (see git history).
//
// Neither source has any live code left in this repo, so any report with
// one of these `source` values is old data that will never be refreshed —
// removing them stops them cluttering the Review Queue and the Admin
// Console's stats widgets (Top source, Reports by Source, etc).
//
...
const SOURCES_TO_REMOVE = ["Social Media (Reddit)", "Social Media (Mastodon)"];
